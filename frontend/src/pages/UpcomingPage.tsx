...
import { getUpcomingEvents, type EventCardProps } from '../utils/api';
...
const UpcomingEventsPage = () => {
  const { data: eventsData, isLoading, error, request: fetchUpcomingEvents } = useApi(getUpcomingEvents);
  const [searchTerm, setSearchTerm] = useState('');
  const [currentPage, setCurrentPage] = useState(1);
  const debouncedSearchTerm = useDebounce(searchTerm, 500);

  useEffect(() => {
    fetchUpcomingEvents({ search: debouncedSearchTerm, page: currentPage, limit: 8 });
